...
**Estado:** plan cerrado 2026-09-21. **S1 ✅ `cea31b5` · S2 ✅ `750556e` · S3 ✅ `6a1ddb7`** (deps `24b8232`) — gate completo verde (**191 passed / 10 skipped** viewport-gated). S4 pendiente, S5 es el cierre documental. Siguiente: **S4 (FAQs `/faqs`)**, y ojo que incluye alinear el teaser de la landing a serif (R8), lo que toca `src/pages/index.astro` y sus specs. Track bajo `size:exception` aprobado (ver Forecast). Nota de proceso: dos delegaciones de S1 y S3 murieron por fallos del runtime/proveedor y **dejaron archivos escritos sin verificar** — revisar el árbol ante cada aborto, ver Progreso (c) y (f).
